...
__global__ void mutual_information(
	MY_PIXEL *ref_d,
	MY_PIXEL *flt_d,
	data_t *j_h_d,
	data_t *href_d,
...
void mutual_information_master(MY_PIXEL *ref, MY_PIXEL *flt, MY_PIXEL *ref_d, MY_PIXEL *flt_d, data_t *j_h_d,
							   data_t *href_d, data_t *hflt_d, data_t *result_d, data_t *result, size_t n_images, double *kernel_only_time)
{
	...
	Timer timer;
	timer.start();
	mutual_information<<<n_images, GPU_THREADS, 0>>>(ref_d, flt_d, j_h_d, href_d, hflt_d, result_d);
